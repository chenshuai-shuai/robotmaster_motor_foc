...
/*软件I2C时序参数：F4@168MHz 下无延时位带时 SCL 实际超过 1MHz，
 *远超 SH1106 上限 400kHz，从机无法正常应答（屏幕无显示）。
 *每次引脚翻转后插入 DWT 半周期延时，总线约 250kHz。*/
...
/*DWT 周期计数器微秒延时：不依赖 SysTick/中断，FreeRTOS 临界区内外均安全。
 *首次调用时自动使能 CYCCNT；两次采样差值用无符号减法计算，无回卷问题。*/
...
void OLED_W_SCL(uint8_t BitValue)
{
	...
	OLED_Delay_us(OLED_I2C_HALF_PERIOD_US);	//控制SCL在SH1106上限400kHz以内
}
...
void OLED_GPIO_Init(void)
{
	...
	/*注意：本函数在 FreeRTOS 调度器启动前被调用（main 中），此阶段先前的任务创建
	 *（xTaskCreate）会因 uxCriticalNesting 初值 0xaaaaaaaa 无法恢复 BASEPRI，
	 *SysTick 被屏蔽，HAL_Delay 会永久卡死（实测整板冻结）；
	 *故改用 DWT 周期计数器做 100ms 忙等待，不依赖任何中断。*/
...
{
	uint8_t i;
	static uint8_t s_ack_fail_logged = 0U;	/*I2C无应答只打印一次*/
	...
	/*第9个时钟为应答位：释放SDA后读回总线电平，开漏输出写1时IDR反映总线真实电平；
	 *0=从机ACK（屏在线），1=NACK（接线/外部上拉/供电/地址错误），仅首次打印一次。*/
	OLED_W_SDA(1);
	OLED_W_SCL(1);
	if (HAL_GPIO_ReadPin(OLED_SDA_GPIO_Port, OLED_SDA_Pin) != GPIO_PIN_RESET)
	{
...
{
	static uint8_t s_first_command = 1U;	/*调试日志：仅首次调用时打印一次*/
