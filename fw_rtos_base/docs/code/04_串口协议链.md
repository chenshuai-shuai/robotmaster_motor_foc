...
- 协议设计文档 `docs/1_规则（既定事实）/协议_串口控制_v1.md` 与实现存在**已知偏差**（例如文档 §5.1 写 `#CLR` 回 `@OK CLR err=0xNN`，实现走异步握手回 `@OK CLR done` + `@EVT CLRERR err=0xNN`；文档 `#VER` 的 `gear=8` 字段实现里没有）。**本文以代码为准**。
