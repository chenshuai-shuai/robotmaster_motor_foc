...
| 你的目的 | 读什么 |
|---|---|
| 我要**用**它（发命令、看屏幕、测试） | ⑨ `docs/1_规则（既定事实）/操作手册_串口命令.md` → ⑩ `docs/1_规则（既定事实）/测试手册_8108串口控制.md` |
...
├─ 1_规则（既定事实）/协议_串口控制_v1.md               ← 串口协议定稿（设计文档）
├─ 1_规则（既定事实）/操作手册_串口命令.md               ← 面向使用者：发什么 → 收什么 → 电机做什么
└─ 1_规则（既定事实）/测试手册_8108串口控制.md           ← 每次改动后的验收清单（TM-1…TM-7，含逐字预期）
```
...
| 13 | **修改必同步测试手册** | 每次改动写明"重点+逐字预期"，否则无法验收 | `docs/1_规则（既定事实）/测试手册_8108串口控制.md` TM-1…TM-7 |
...
*本文由工程实测数据 + 源码阅读整理；对应的验收清单与逐字预期见 `docs/1_规则（既定事实）/测试手册_8108串口控制.md`。*
